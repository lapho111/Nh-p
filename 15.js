const protobuf = require('protobufjs'); // Giả sử protobuf được import đúng cách
const spotifyJson = {
...
if (resStatus !== 200) {
    ...
    if (url.includes("bootstrap/v1/bootstrap") && method === POST_METHOD) {
        const bootstrapResponseType = protobuf.Root.fromJSON(spotifyJson).lookupType("BootstrapResponse");
    ...
    } else if (url.includes("user-customization-service/v1/customize") && method === POST_METHOD) {
        const ucsResponseWrapperType = protobuf.Root.fromJSON(spotifyJson).lookupType("UcsResponseWrapper");
        ...
        $notification.post('Spotify Unlock Premium', "Đường dẫn/phương thức yêu cầu không khớp:", `${method}, ${url}`);
    }
...
// Function to process the accountAttributesMapObj
function processMapObj(accountAttributesMapObj) {
    ...
    accountAttributesMapObj['nft-disabled'] = { stringValue: '1' }; // Hide premium ad tab
    ...
    accountAttributesMapObj['com.spotify.madprops.use.ucs.product.state'] = { boolValue: true }; // Enable customization
